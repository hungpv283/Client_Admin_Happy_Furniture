...
import { deleteAssembly, getAssemblies } from "@/lib/api";
import type { Assembly, AssemblyFilters } from "@/lib/api";
...
export default function AssembliesTable() {
  ...
  const fetchAssemblies = useCallback(async () => {
    setLoading(true);
    try {
      const data = await getAssemblies(page, pageSize, appliedFilters);
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-800 dark:text-white/90">Assemblies</h1>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Tổng cộng {totalCount} assembly
          </p>
        </div>
        <Link
      ...
      <div className="mb-6 rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
        <div className="mb-4">
          ...
          <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Lọc assembly theo tên và trạng thái.</p>
        </div>
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <div>
            <label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Tên assembly</label>
            <input
              type="text"
              value={filters.name}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50 dark:border-gray-800 dark:bg-white/[0.02]">
                ...
                <th className="px-5 py-4 text-left font-semibold text-gray-600 dark:text-gray-400">Code</th>
            ...
            <tbody>
              {loading ? (
                ...
                assemblies.map((assembly) => (
                  ...
                  >
                    <td className="px-5 py-4 text-gray-500 dark:text-gray-400">#{assembly.id}</td>
                    <td className="px-5 py-4 font-medium text-gray-800 dark:text-white/90">{assembly.nameVi}</td>
                    <td className="px-5 py-4 text-gray-500 dark:text-gray-400">{assembly.nameEn || "-"}</td>
                    <td className="px-5 py-4 text-gray-500 dark:text-gray-400">
                      <span className="rounded bg-gray-100 px-2 py-0.5 font-mono text-xs dark:bg-white/10">{assembly.code}</span>
                    </td>
                    <td className="px-5 py-4 text-gray-500 dark:text-gray-400">
                      <span className="line-clamp-2">{assembly.descriptionVi || "-"}</span>
                    </td>
                    <td className="px-5 py-4 text-gray-500 dark:text-gray-400">
                      <span className="line-clamp-2">{assembly.descriptionEn || "-"}</span>
                    </td>
                    <td className="px-5 py-4">
                      ...
                      >
                        <span className={`h-1.5 w-1.5 rounded-full ${assembly.isActive ? "bg-green-500" : "bg-gray-400"}`} />
                        {assembly.isActive ? "Hoạt động" : "Ẩn"}
                      </span>
                    </td>
                    <td className="px-5 py-4 text-gray-500 dark:text-gray-400">
                      {new Date(assembly.createdAt).toLocaleDateString("vi-VN")}
                    </td>
                    <td className="px-5 py-4">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          onClick={() => openDeleteConfirm(assembly.id, assembly.nameVi)}

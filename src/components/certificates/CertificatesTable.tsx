...
import type { Certificate, CertificateFilters } from "@/lib/api";
...
export default function CertificatesTable() {
  ...
  const fetchCertificates = useCallback(async () => {
    setLoading(true);
    try {
      const data = await getCertificates(page, pageSize, appliedFilters);
      setItems(data.items);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      success(`Đã xóa chứng chỉ "${deleteTarget.nameVi}"`);
  ...
  return (
    <div>
      <ConfirmDialog
        open={confirmOpen}
        title="Xóa chứng chỉ"
        message={`Bạn có chắc muốn xóa chứng chỉ "${deleteTarget?.nameVi}"? Hành động này không thể hoàn tác.`}
        confirmLabel="Xóa"
        loading={deleting}
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-800 dark:text-white/90">Chứng chỉ</h1>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Tổng cộng {totalCount} chứng chỉ
          </p>
        </div>
        <Link
      ...
      <div className="mb-6 rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03]">
        <div className="mb-4">
          ...
          <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Lọc chứng chỉ theo tên và trạng thái.</p>
        </div>
        <div className="grid gap-4 md:grid-cols-2">
          <div>
            <label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Tên chứng chỉ</label>
            <input
              type="text"
              value={filters.name}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50 dark:border-gray-800 dark:bg-white/[0.02]">
                ...
                <th className="px-5 py-4 text-left font-semibold text-gray-600 dark:text-gray-400">Logo</th>
                  ...
                  >
                    ...
                    <td className="px-5 py-4">
                      {item.logoUrl ? (
                        <img
                          src={item.logoUrl}
                          alt={item.nameVi}
                          className="h-8 w-auto rounded object-contain bg-gray-50 dark:bg-white/5"
                        />
                      ) : (
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center justify-end gap-2">
                        <Link
                          href={`/certificates/${item.id}/edit`}
